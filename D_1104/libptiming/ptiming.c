...

{
  ...
  return (double)( local_cyc - last_cyc );
}
...
{
  uint64_t local_cyc = read_cyc_counter();
  double result = local_cyc - last_cyc - counter_overhead;
...
#define N_SHOTS 4
  ...
  double result[N_SHOTS];
  for ( int i = 0; i < N_SHOTS; i++) {
    start_counter();
    get_counter();
    result[i] = get_counter();
  }
  for ( int i = 1; i < N_SHOTS; i++)
    result[0] += result[i];
  
  return (counter_overhead = result[0] / N_SHOTS);
}
...
#define  N_CALIBRATION 100
  ...
  clock_info.clock_overhead      = clock_overhead;
