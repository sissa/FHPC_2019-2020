...
MPICC=$DEST/bin/mpicc
MPICXX=$DEST/bin/mpicxx
MPIFC=$DEST/bin/mpif90
...
HDFGROUP=http://www.hdfgroup.org/ftp/HDF5/releases/hdf5-${hdf5_ver}/src/
ZLIB=http://zlib.net
...
cd hdf5-${hdf5_ver}
echo ./configure CC="$MPICC" CXX="$MPICXX" FC="$MPIFC" \
        --prefix=$DEST --with-zlib=$DEST --enable-parallel \
        --disable-cxx --enable-shared \
        --disable-fortran >> $DEST/logs/configure.log
./configure CC="$MPICC" CXX="$MPICXX" FC="$MPIFC" \
	--prefix=$DEST --with-zlib=$DEST --enable-parallel \
        --enable-shared --disable-cxx \
        --disable-fortran >> $DEST/logs/configure.log 2>&1
$MAKE >> $DEST/logs/compile.log 2>&1 && \
  $MAKE install >> $DEST/logs/install.log 2>&1
if [ $? -ne 0 ]
then
...
echo ./configure CC="$MPICC" FC="$MPIFC" --prefix=$DEST --enable-netcdf-4 \
  CPPFLAGS=-I$DEST/include LDFLAGS=-L$DEST/lib LIBS="$H5LIBS" \
  --disable-dap >> $DEST/logs/configure.log
./configure CC="$MPICC" FC="$MPIFC" --prefix=$DEST --enable-netcdf-4 \
  CPPFLAGS=-I$DEST/include LDFLAGS=-L$DEST/lib LIBS="$H5LIBS" \
...
echo ./configure PATH=$DEST/bin:$PATH CC="$MPICC" FC="$MPIFC" \
     CPPFLAGS=-I$DEST/include LDFLAGS=-L$DEST/lib --prefix=$DEST \
     >> $DEST/logs/configure.log
./configure PATH=$DEST/bin:$PATH CC="$MPICC" FC="$MPIFC" F77="$MPIFC" \
     CPPFLAGS=-I$DEST/include LDFLAGS=-L$DEST/lib --prefix=$DEST \
     >> $DEST/logs/configure.log 2>&1
$MAKE >> $DEST/logs/compile.log 2>&1 && \
  $MAKE install >> $DEST/logs/install.log 2>&1
