...
!
! Parallel write of a netCDF file using netCDF library
!
! Author    : Graziano Giuliani <ggiulian@ictp.it>
! Institute : ICTP
! Date      : Fri Nov 21 09:25:36 CET 2014
!
program collect_and_write
  ...
  integer , dimension(2) :: wdstart , wdcount
  integer :: n , nn
  ...
  ! Open output file
  imode = nf90_mpiio
  imode = ior(imode,nf90_clobber)
  imode = ior(imode,nf90_netcdf4)
  ierr = nf90_create(outfname,imode,ncid,comm=cartcomm,info=mpi_info_null)
  ...
  ! Write global variable from local memory
  ...
  wdcount(1) = nix(nn)
  wdcount(2) = njy(nn)
  ierr = nf90_put_var(ncid,ivar,grid_local,wdstart,wdcount)
  call check_nc_error(ierr,__FILE__,__LINE__)
