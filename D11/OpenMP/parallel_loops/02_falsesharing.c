...
#pragma omp critical
  ...
  double th_min_time = 0;                                   // this will be the min thread time
...
#pragma omp parallel shared(S)
  {    
    int    me       = omp_get_thread_num();
    double mytstart = CPU_TIME_th; 
#pragma omp for
    for ( int ii = 0; ii < N; ii++ )
      S[me] += array[ii];
    double myt      = CPU_TIME_th - mytstart;;
    th_avg_time += myt;
    th_min_time  = ( myt < th_min_time )? myt : th_min_time;
  }
